...
class PopularCell: UITableViewCell {
    ...
    func configureCell(popular: PopularResult) {
        let poster = URL(string: posterURL + popular.poster_path)
        posterImageView.kf.setImage(with: poster!)
        titleLabel.text = popular.title
        releaseLabel.text = popular.release_date
        overviewLabel.text = popular.overview
    }
